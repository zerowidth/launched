...
func (p LaunchdPlist) PlistXML(sourceURL string) string {
	...
	out := buf.String()
	if sourceURL != "" {
		// The encoder writes the declaration and DOCTYPE first, so splice it in
		// just before the plist element instead.
		comment := "<!-- generated by launched " + sourceURL + " -->\n"
		if i := strings.Index(out, "<plist "); i >= 0 {
			out = out[:i] + comment + out[i:]
		}
	}
	return out
}
